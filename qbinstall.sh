...
echo "#############################################################"
echo
echo "国内服务器初次使用请修改github链接"
echo "脚本经gymbao修改，仅保存原作者版权"
...
# Install libtorrent
cd ~
wget --no-check-certificate #原链接为https://github.com/arvidn/libtorrent/releases/download/libtorrent-1_1_9/libtorrent-rasterbar-1.1.9.tar.gz https://blog.jjb.buzz/proxy/libtorrent-rasterbar-1.1.9.tar.gz
if [ $? -eq 0 ];then
...
ln -sf /usr/lib/pkgconfig/libtorrent-rasterbar.pc /usr/lib64/pkgconfig/libtorrent-rasterbar.pc
ln -sf /usr/lib/libtorrent-rasterbar.so.9 /usr/lib64/libtorrent-rasterbar.so.9
if [ $? -eq 0 ];then
...
# Install qBittorrent
cd ~
wget --no-check-certificate #此处需要替换链接 原链接为https://github.com/qbittorrent/qBittorrent/archive/release-4.1.1.tar.gz https://blog.jjb.buzz/proxy/release-4.1.1.tar.gz
if [ $? -eq 0 ];then
